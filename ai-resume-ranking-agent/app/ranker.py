...
from app.agent import extract_requirements, evaluate_candidate
...
def rank_candidates(
    ...
    2. For each candidate, run the agent evaluation loop:
    ...
    # ── Step 2: Evaluate each candidate using agent loop ─────────────────
    candidate_results = []
    for i, name in enumerate(candidate_names, 1):
        logger.info("")
        logger.info("━" * 70)
        logger.info("CANDIDATE %d/%d: %s", i, len(candidate_names), name)
        logger.info("━" * 70)
