    ...
    data.candidates.forEach(c => {
      ...
      card.innerHTML = `
        ...
        <div class="score-bars">
          ${scorebar('Skills', c.score_breakdown.skill_match_score)}
          ${scorebar('Experience', c.score_breakdown.experience_relevance_score)}
          ${scorebar('Projects', c.score_breakdown.project_quality_score)}
          ${scorebar('Education', c.score_breakdown.education_score)}
        </div>

        <!-- Skills Tags -->
        ${c.score_breakdown.matched_skills.length ? `
        <div class="tags-section">
          <div class="tags-label">Matched Skills</div>
          <div class="tags-wrap">${c.score_breakdown.matched_skills.map(s => `<span class="tag matched">${s}</span>`).join('')}</div>
        </div>` : ''}
        ${c.score_breakdown.missing_skills.length ? `
        <div class="tags-section">
          <div class="tags-label">Missing Skills</div>
          <div class="tags-wrap">${c.score_breakdown.missing_skills.map(s => `<span class="tag missing">${s}</span>`).join('')}</div>
        </div>` : ''}
