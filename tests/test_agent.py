...
class TestSessionStore:
    def test_save_and_retrieve(self, strong_resume, sample_jd):
        session_store.save("test-123", sample_jd, [strong_resume])
        assert session_store.session_exists("test-123")
        assert session_store.get_job_description("test-123").role_title == "Senior Python Developer"
        assert len(session_store.get_resumes("test-123")) == 1
    ...
    def test_missing_session(self):
        ...
        assert session_store.get_resumes("nonexistent-id") is None

    def test_delete(self, strong_resume, sample_jd):
        session_store.save("del-test", sample_jd, [strong_resume])
...
class TestUploadEndpoint:
    ...
    def test_too_many_resumes_returns_400(self, client):
        ...
        with patch("app.routers.upload.extract_jd_features") as mock_jd, \
             patch("app.routers.upload.extract_text_from_file") as mock_parse:
            mock_jd.return_value = MagicMock(role_title="Dev")
    ...
    @patch("app.routers.upload.extract_jd_features")
    @patch("app.routers.upload.extract_text_from_file")
    @patch("app.routers.upload.extract_resume_features")
    def test_successful_upload(self, mock_resume, mock_parse, mock_jd, client):
        mock_jd.return_value = ExtractedJobDescription(
            raw_text="Python dev needed",
            required_skills=["Python"],
            role_title="Python Developer",
        )
        mock_parse.return_value = "resume text"
        mock_resume.return_value = ExtractedResume(
            candidate_name="Alice", filename="alice.pdf", raw_text="resume text",
        )
...
class TestRankEndpoint:
        ...
        mock_rank.return_value = RankResponse(
            ...
            candidates=[
                RankedCandidate(
                    ...
                    score=86.0, score_breakdown=mock_score,
    ...
    def test_rank_response_schema(self, client, strong_resume, weak_resume, sample_jd):
        """Integration: verify full response schema without mocking ranker."""
        session_store.save("schema-test", sample_jd, [strong_resume, weak_resume])

        with patch("app.ranker.generate_explanation") as mock_exp, \
             patch("app.ranker.generate_ranking_summary") as mock_sum:
            mock_exp.return_value = (["Strength"], ["Weakness"], "Explanation text.")
